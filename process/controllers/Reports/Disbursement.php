...
include("../config/dbconn.php");
  ...
  if($credit == 1){
    $header1 .= '<th colspan = "3">CREDIT</th>  ';
  ...
  if($debit == 1){
    $header1 .= '<th colspan = "5">DEBIT</th>';
    $header2 .= '<th></th>
      <th colspan = "2">SUNDRY</th>
      <th></th>
      <th></th>';
    $header3 .= '<th>ACCOUNTS PAYABLE</th>
      <th>ACCOUNT</th>
      <th>AMOUNT</th>
      <th>PARTICULARS</th>
      <th>OUTPUT VAT</th>';
  ...
  $query = "call get_disbursement('".$frmdate."','".$todate."');"; 
...
// SPEND MONEY
   /* $output.='<table border = "1">';
  $query2 = "call get_disbursement2('".$frmdate."','".$todate."');"; 
        $result2 = $conn->query($query2);
          while ($row2 = $result2->fetch_assoc()) 
          {
            ...
            if($checkno == 1){
                $newcheckno = "";
                if(strpos($row2["checkno"], '|') !== false){
                  $explodestring = explode('|', str_replace(' ', '', $row2["checkno"]));
                  $newcheckno = $explodestring[0];
                }else{
                  $newcheckno = $row2["checkno"];
                }
                $output .= '  <td>'.$newcheckno.'</td>  ';
              } 
            if($voucherref == 1){
